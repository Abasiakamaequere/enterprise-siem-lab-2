...
```text
Build endpoint telemetry architecture documentation
````
...
## Then we start the actual telemetry files

After the README, we'll create them in this order:

```text
telemetry/
│
├── README.md
├── 01-windows-endpoint.md
├── 02-sysmon.md
├── 03-universal-forwarder.md
├── 04-forwarder-configuration.md
├── 05-splunk-receiver.md
└── 06-telemetry-validation.md
```

This mirrors what we did for Deployment and gives the recruiter a clear progression from **infrastructure → endpoint → collection → transport → SIEM ingestion → validation**.

**Do only the `telemetry/README.md` for now.**

When you've committed it, tell me:

**`Telemetry README done`**

and I'll give you **`01-windows-endpoint.md`**.
